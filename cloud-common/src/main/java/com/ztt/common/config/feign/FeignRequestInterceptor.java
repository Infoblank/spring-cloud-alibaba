...
public class FeignRequestInterceptor implements RequestInterceptor {
	...
	public void customHeaders(RequestTemplate template) {
		...
		template.header(CommonConstant.REQ_ID, RequestIdUtils.getRequestId());
		String spanId = MDC.get(CommonConstant.SPAN_ID);
		if (Objects.nonNull(spanId)) {
			template.header(CommonConstant.SPAN_ID, spanId);
			log.info("feign add header spanId:{}", spanId);
		}
		String traceId = MDC.get(CommonConstant.TRACE_ID);
		if (Objects.nonNull(traceId)) {
			template.header(CommonConstant.TRACE_ID, traceId);
			log.info("feign add header traceId:{}", traceId);
		}
